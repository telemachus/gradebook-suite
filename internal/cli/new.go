...
func (cmd *cmdEnv) newGradebook(class *gradebook.Class, cfg newCfg) {
	...
	assignmentRecords := make(gradebook.AssignmentRecords, 0, len(emails))
	for _, email := range emails {
		assignmentRecords = append(assignmentRecords, &gradebook.AssignmentRecord{Email: email, Grade: nil})
	}
